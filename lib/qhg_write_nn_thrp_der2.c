...
void
qhg_write_nn_thrp_der2(char fname[], qhg_thrp_correlator corr_thrp, char group[])
{
      ...
      for(int ichan=0; ichan<NCHAN; ichan++) {
        for(int t0=0; t0<ldims[0]; t0++) {
          ...
          for(unsigned long int v3=0; v3<lv3; v3++) {
            ...
            buf[0 + 2*v0] = creal(c[TOT_IDX(v, mu, nu, ichan)]);
            buf[1 + 2*v0] = cimag(c[TOT_IDX(v, mu, nu, ichan)]);
          }
        }

        char *group_tag;
        asprintf(&group_tag, "%s%s", chan_tags[ichan], der_tags[mu][nu]);
